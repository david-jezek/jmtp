    ...
    static PortableDeviceObjectImplWin32 convertToPortableDeviceObject(String objectID, 
    	...
    	try {
    		...
    		synchronized (lock) {
    			...
    			values = device.getProperties().getValues(objectID, keyCollection);
			}
            ...
            if(contentType.equals(Win32WPDDefines.WPD_CONTENT_TYPE_FOLDER)) {
                return new PortableDeviceFolderObjectImplWin32(objectID, device);
            }
            else if(contentType.equals(Win32WPDDefines.WPD_CONTENT_TYPE_AUDIO)) {
            	return new PortableDeviceAudioObjectImplWin32(objectID, device);
            }
            else if(contentType.equals(Win32WPDDefines.WPD_CONTENT_TYPE_PLAYLIST)) {
            	return new PortableDevicePlaylistObjectImplWin32(objectID, device);
            }
            	...
            	if(category.equals(Win32WPDDefines.WPD_FUNCTIONAL_CATEGORY_STORAGE)) {
            		return new PortableDeviceStorageObjectImplWin32(objectID, device);
            	}
            	else if(category.equals(Win32WPDDefines.WPD_FUNCTIONAL_CATEGORY_RENDERING_INFORMATION)) {
            		return new PortableDeviceRenderingInformationObjectImplWin32(objectID, device);
            	}
            	else {
            		//basis object terug geven
            		return new PortableDeviceObjectImplWin32(objectID, device);
            	}
            }
            else {
                return new PortableDeviceObjectImplWin32(objectID, device);
